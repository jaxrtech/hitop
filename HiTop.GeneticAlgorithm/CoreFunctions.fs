...
module Organism =
    ...
    let evaluate (target: BinaryReader) (organism: Organism) : Fitness =
        ...
        let correct = score 0 engine |> float
        let length = target.BaseStream.Length |> float

        correct / length

    let toEvaluated (target: BinaryReader) (organism: Organism) : EvaluatedOrganism =
        (organism, organism |> evaluate target)
...
module Population =
    ...
    let create populationCount programSize : Population =
        Array.init populationCount (fun _ -> Organism.createRandom programSize)

    let evaluate (target: BinaryReader) (population: Population) : EvaluatedPopulation =
        population
        |> Array.map (Organism.toEvaluated target)
